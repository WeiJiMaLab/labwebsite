...
function formatPeople(person, index) {
    return (
        <div>
            <Row key={index} className="person-card desktoponly" style={{ alignItems: 'flex-start', marginBottom: '20px' }}>
            <Col xs={3}>
                {person.image ? (
                    <div style={{ 
                    width: '100%', 
        ...
        <Layout>
            <section id="people">
                <div className="banner people-list">
                    ...
                    />
                    <h2 style={{ position: 'sticky', top: '0px', backgroundColor: 'var(--background-color)', zIndex: 10 }}>Lab Members</h2>
                    {filterPeople(currentLabMembers).map((person, index) => formatPeople(person, index))}

                    <h2 style={{ position: 'sticky', top: '0px', backgroundColor: 'var(--background-color)', zIndex: 10 }}>Alumni Collaborators</h2>
                    {filterPeople(currentAlumniCollaborators).map((person, index) => formatPeople(person, index))}

                    <h2 style={{ position: 'sticky', top: '0px', backgroundColor: 'var(--background-color)', zIndex: 10 }}>External Collaborators</h2>
                    {filterPeople(currentExternalCollaborators).map((person, index) => formatPeople(person, index))}

                    <h2 style={{ position: 'sticky', top: '0px', backgroundColor: 'var(--background-color)', zIndex: 10 }}>Past Lab Members</h2>
                    {filterPeople(pastLabMembers).map((person, index) => formatPeople(person, index))}

                    <h2 style={{ position: 'sticky', top: '0px', backgroundColor: 'var(--background-color)', zIndex: 10 }}>Past Collaborators</h2>
